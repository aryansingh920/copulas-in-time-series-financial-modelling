...
def compare_copula_fits(data: pd.DataFrame):
    ...
    def sample_empirical_copula(n_samples_to_draw):
        ...
        indices = np.random.choice(
            n_samples, size=n_samples_to_draw, replace=True)
    ...
    def calculate_energy_score(x_samples, y_samples):
        ...
        # Multiply by 2 because we only counted each pair once
        second_term *= 2 / (n_x * (n_x - 1))
    ...
    for name, copula in ts_copulas.items():
        ...
        try:
            ...
            if name == 'DCC':
                ...
                copula.fit(uniform_df, copula_type='t',
                           copula_params={'df': 4})
            else:
                copula.fit(uniform_df)

            # Get copula simulations
            simulated = copula.simulate(n_samples)

            # Convert to numpy array if returned as DataFrame
            if isinstance(simulated, pd.DataFrame):
                simulated = simulated.values

            # Check if simulated values are in [0,1] range
            # If not, apply probability integral transform to get uniform margins
            min_val = np.min(simulated)
            max_val = np.max(simulated)

            if min_val < 0 or max_val > 1:
                print(
                    f"Transforming non-uniform outputs to uniform margins (original range: [{min_val:.4f}, {max_val:.4f}])")

                # Apply probability integral transform to each column
                uniform_simulated = np.zeros_like(simulated)
                for j in range(simulated.shape[1]):
                    # Use empirical CDF transform (rank transform)
                    uniform_simulated[:, j] = stats.rankdata(
                        simulated[:, j]) / (n_samples + 1)

                simulated = uniform_simulated
                print(
                    f"After transformation: range [{np.min(simulated):.4f}, {np.max(simulated):.4f}]")
    ...
    colors = ['#3498db' if results[name]['type'] ==
              'basic' else '#e74c3c' for name in names]
